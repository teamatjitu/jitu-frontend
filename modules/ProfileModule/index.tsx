...
export default function ProfileModule({ data, session }: { data: ProfileData | null, session: any }) {
              ...
              {stats.totalTryout === 0 && (
                <div className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-xl p-6 text-white shadow-lg">
                  <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                    <div>
                      <h3 className="text-xl font-bold mb-1">Belum ada riwayat belajar?</h3>
                      <p className="text-blue-100 text-sm">Yuk kerjakan Tryout pertamamu untuk mengukur kemampuan!</p>
                    </div>
                    <Button variant="secondary" onClick={() => router.push('/tryout')} className="whitespace-nowrap">
                      Coba Tryout Gratis
                    </Button>
                  </div>
                </div>
              ...
              <CardContent>
                {attempts.length > 0 ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-12 text-center">
                    ...
                    <h3 className="font-semibold text-slate-900">Belum Ada Riwayat</h3>
                    <p className="text-slate-500 text-sm max-w-xs mt-1 mb-4">
                      Kamu belum mengerjakan tryout apapun. Mulai sekarang untuk melihat progresmu.
                    </p>
                    <Button onClick={() => router.push('/tryout')}>Mulai Tryout</Button>
                  </div>
                )}
              </CardContent>
            </Card>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <div className="p-2 border rounded-md bg-slate-50 text-slate-500 text-sm flex items-center justify-between">
                      {user.email}
                      <Badge variant="outline" className="text-green-600 bg-green-50 border-green-200">Terverifikasi</Badge>
                    </div>
                  </div>
                </div>
    ...
    <div className="flex items-center justify-between p-3 rounded-lg border border-slate-100 hover:border-blue-200 hover:bg-blue-50/30 transition-all group">
      ...
      <div className="text-right">
        ...
        <span className={`text-[10px] uppercase font-bold px-1.5 py-0.5 rounded ${
          attempt.score >= 700 ? "bg-green-100 text-green-700" : "bg-slate-100 text-slate-600"
        }`}>
          {attempt.status}
        </span>
      </div>
